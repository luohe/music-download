...
// 搜索
const ifkdyMusicList = async (name, author) => {
  try {
    ...
    const musicUrlList = data
    .filter(item => item.title.includes(name) && item.author.includes(author))
    .map(item => ({
      author: item.author,
      title: item.title,
      url: item.url,
...
// 爬音乐并下载
const grabMusic = async (name, author) => {
  const musicList = await ifkdyMusicList(name, author);
